...
export default function EditClassClient({ id }: Props) {
    const router = useRouter();

    // Data state
    const [originalData, setOriginalData] = useState<ClassItem | null>(null);
    ...
    useEffect(() => {
        ...
        const loadAll = async () => {
            try {
                ...
                // Fetch class details and categories in parallel
                ...
                // Set class data
                const d = classRes.data?.data ?? (classRes.data as unknown as ClassItem);
                if (!d) throw new Error("Data kelas tidak ditemukan.");

                setOriginalData(d);
                setName(d.name ?? "");
                setSubtitle(d.subtitle ?? "");
                setDescription(d.description ?? "");
                setDescCharCount((d.description ?? "").length);

                // Safe extract category IDs whether stored as string IDs or populated objects
                const catIds = (d.categories ?? []).map((c) =>
                    typeof c === "string" ? c : (c?._id ?? String(c))
                );
                setSelectedCategories(catIds);

                setFeatures(
                    d.features && d.features.length > 0 ? d.features : ["", "", ""]
                );
                setHighlight(Boolean(d.highlight));
                setColor(d.color || "#0F172A");
            } catch (err: unknown) {
                if (!isMounted) return;
                const e = err as { response?: { data?: { message?: string } }; message?: string };
                const msg = e?.response?.data?.message || e?.message || "Gagal memuat data kelas.";
                setFetchError(msg);
                setIsLoadingCategories(false);
            } finally {
    ...
    const isDirty =
        ...
        subtitle.trim() !== (originalData.subtitle ?? "") ||
        description.trim() !== (originalData.description ?? "") ||
        highlight !== Boolean(originalData.highlight) ||
        color !== (originalData.color || "#0F172A") ||
        JSON.stringify([...selectedCategories].sort()) !==
            JSON.stringify(
                (originalData.categories ?? []).map((c) =>
                    typeof c === "string" ? c : (c?._id ?? String(c))
                ).sort()
            ) ||
        JSON.stringify(features.filter((f) => f.trim())) !==
            JSON.stringify((originalData.features ?? []).filter((f) => f.trim()));
    ...
    const handleReset = () => {
        ...
        const catIds = (originalData.categories ?? []).map((c) =>
            typeof c === "string" ? c : (c?._id ?? String(c))
        );
        setSelectedCategories(catIds);
        setFeatures(
            originalData.features && originalData.features.length > 0
                ? originalData.features
                : ["", "", ""]
        );
        setHighlight(Boolean(originalData.highlight));
        setColor(originalData.color || "#0F172A");
        setErrors({});
        setTouched({});
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await Swal.fire({
                ...
                html: `<p class="text-sm text-slate-600">Perubahan pada program kelas <strong>${payload.name}</strong> telah tersimpan.</p>`,
                confirmButtonColor: "#6366f1",
                confirmButtonText: "Kembali ke Daftar Kelas",
            });
            router.push("/class");
        } catch (err: unknown) {
            const e = err as { response?: { data?: { message?: string } }; message?: string };
            Swal.fire({
                ...
                text: e?.response?.data?.message || e?.message || "Terjadi kesalahan saat menyimpan perubahan.",
                confirmButtonColor: "#6366f1",
            });
        } finally {
    ...
    const handleDiscard = () => {
        ...
        Swal.fire({
            ...
            if (r.isConfirmed) router.push("/class");
        });
    };
            ...
            <div className="max-w-5xl mx-auto space-y-8 animate-fadeIn">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
                    <div className="xl:col-span-3 space-y-6">
                        <div className="rounded-3xl border border-slate-200/80 dark:border-white/10 bg-white dark:bg-slate-900/80 p-6 sm:p-8 space-y-6">
                    ...
                    <div className="xl:col-span-2 space-y-6">
                        <div className="rounded-3xl border border-slate-200/80 dark:border-white/10 bg-white dark:bg-slate-900/80 p-6 space-y-6">
                            <Skeleton className="w-40 h-5 mb-4" />
    ...
    if (fetchError) {
        return (
            <div className="max-w-lg mx-auto mt-16 p-8 rounded-3xl border border-red-200 bg-red-50 dark:bg-red-500/10 dark:border-red-500/20 text-center space-y-4 animate-fadeIn">
                <div className="w-12 h-12 rounded-2xl bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400 mx-auto flex items-center justify-center">
                    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                </div>
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">Gagal Memuat Kelas</h2>
                <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">{fetchError}</p>
                <button
                    type="button"
                    onClick={() => router.push("/class")}
                    className="px-6 py-2.5 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-xs font-bold shadow transition-all cursor-pointer hover:bg-slate-800"
                >
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 animate-fadeIn">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold uppercase tracking-widest text-slate-400">Edit Program</span>
                        </div>
                ...
                {isDirty && (
                    <div className="flex items-center gap-2 self-start sm:self-auto">
                        <span className="px-3 py-1 rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20 text-xs font-semibold animate-pulse">
                            ● Ada perubahan belum disimpan
                        </span>
                        <button
                            type="button"
                            onClick={handleReset}
                            className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 text-slate-600 dark:text-slate-300 text-xs font-semibold transition-all cursor-pointer"
                        >
                            Reset
                        </button>
                    </div>
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-6">
                {/* ── Live Preview Strip ───────────────────────────────────── */}
                <div
                    className="p-4 rounded-2xl border flex flex-wrap items-center justify-between gap-3 transition-all duration-300 shadow-sm"
                    style={{
                        ...
                        <div className="rounded-3xl border border-slate-200/80 dark:border-white/10 bg-white dark:bg-slate-900/80 backdrop-blur p-6 sm:p-8 shadow-sm space-y-6">
                            <div className="flex items-center gap-2.5 pb-4 border-b border-slate-100 dark:border-white/5">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">Identitas program kelas bimbingan</p>
                                </div>
                            </div>
                                    ...
                                    <div key={idx} className="flex items-center gap-2.5">
                                        <div className="flex flex-col gap-1 shrink-0">
                                            <button
                                                ...
                                                disabled={idx === 0}
                                                className="p-1 rounded-lg text-slate-300 dark:text-slate-600 hover:text-slate-500 disabled:opacity-30 transition-colors cursor-pointer"
                                                aria-label="Naikkan"
                                            >
                    ...
                    <div className="xl:col-span-2 space-y-6">
                                ...
                                {selectedCategoryNames.length > 0 && (
                                    <div className="flex flex-wrap gap-1.5 mb-3">
                                        {selectedCategoryNames.map((c) => (
                                            <span
                                                key={c._id}
                                                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-100 dark:bg-indigo-500/15 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/30"
                                            >
                        ...
                        <div className="rounded-3xl border border-slate-200/80 dark:border-white/10 bg-white dark:bg-slate-900/80 backdrop-blur p-6 shadow-sm space-y-5">
                            ...
                            <FormField label="Warna Tema Kelas" error={touched.color ? errors.color : undefined} hint="Warna utama yang merepresentasikan kelas ini">
                                ...
                                <div className="flex items-center gap-2.5">
                                    <div
                                        className="w-10 h-10 rounded-xl border-2 border-slate-200 dark:border-white/10 shrink-0 transition-all"
                                        style={{ backgroundColor: color }}
                                    />
                                    <input
                                        id="field-color"
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-5 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200/80 dark:border-white/10 shadow-sm">
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                        ...
                        <button
                            ...
                            className="flex-1 sm:flex-none px-7 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 text-white font-bold text-sm shadow-[0_4px_20px_rgba(99,102,241,0.3)] hover:shadow-[0_4px_25px_rgba(99,102,241,0.45)] transition-all duration-200 active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none cursor-pointer flex items-center justify-center gap-2.5"
                        >
                            {isSubmitting ? (
                                <>
